...
export default Mapped(({ modalLoginState, pageContext }) => {
  ...
  return (
    <div>
      <Helmet>
        <meta charSet='utf-8' />
        <title>{presets[0].device_name + ' Presets for Bitwig Studio'}</title>
        <meta name='viewport' content='width=device-width' />
        <meta property='og:url' content={'https://bitwig.community/creator-' + urlSlug(fp.user.username)} />
        <meta property='og:title' content={'All' + presets[0].device_name + ' Presets for Bitwig Studio'} />
        <meta property='og:description' content={'Browse and download many many presets for Bitwig Studio created for ' + presets[0].device_name} />
      ...
      <MenuSub />
      <SignIn />
      <SectionHeader h={fp.device_name + ' Presets'}>
      Download Presets for Bitwig Studio for {pageContext.presets[0].device_name}.
